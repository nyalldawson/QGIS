/***************************************************************************
    qgslinedisplacementrendererwidget.cpp
    ---------------------
    begin                : November 2023
...
QgsRendererWidget *QgsLineDisplacementRendererWidget::create( QgsVectorLayer *layer, QgsStyle *style, QgsFeatureRenderer *renderer )
{
  return new QgsLineDisplacementRendererWidget( layer, style, renderer );
}

QgsLineDisplacementRendererWidget::QgsLineDisplacementRendererWidget( QgsVectorLayer *layer, QgsStyle *style, QgsFeatureRenderer *renderer )
  : QgsRendererWidget( layer, style )
{
  ...
  if ( type != Qgis::GeometryType::Line )
  {
    ...
    QLabel *label = new QLabel( tr( "The displaced lines renderer only applies to line layers. \n"
  ...
  setupUi( this );
  connect( mRendererComboBox, static_cast<void ( QComboBox::* )( int )>( &QComboBox::currentIndexChanged ), this, &QgsLineDisplacementRendererWidget::mRendererComboBox_currentIndexChanged );

  // try to recognize the previous renderer
  // (null renderer means "no previous renderer")
  if ( renderer )
  {
    mRenderer.reset( QgsLineDisplacementRenderer::convertFromRenderer( renderer ) );
  }
  if ( ! mRenderer )
  {
    // use default embedded renderer
    mRenderer.reset( new QgsLineDisplacementRenderer( QgsFeatureRenderer::defaultRenderer( type ) ) );
    if ( renderer )
...
QgsLineDisplacementRendererWidget::~QgsLineDisplacementRendererWidget() = default;

QgsFeatureRenderer *QgsLineDisplacementRendererWidget::renderer()
{
...
void QgsLineDisplacementRendererWidget::setContext( const QgsSymbolWidgetContext &context )
{
...
void QgsLineDisplacementRendererWidget::setDockMode( bool dockMode )
{
...
void QgsLineDisplacementRendererWidget::mRendererComboBox_currentIndexChanged( int index )
{
  ...
  if ( m )
  {
    ...
    connect( mEmbeddedRendererWidget.get(), &QgsRendererWidget::widgetChanged, this, &QgsLineDisplacementRendererWidget::widgetChanged );
